"""
table_agent.py — 표 데이터 구조화 에이전트
==========================================
역할: project_input.json의 budget/schedule/team/hire_plan을 DOCX 삽입 가능한 구조로 정규화.

주요 처리:
    budget_mix + budget_total → 표 #5 행/열 매핑 데이터 생성
    schedule (list) → 월별 간트 + 수행내용 + 비중 정규화
    team (list) → 참여인력 표 데이터
    hire_plan (list) → 추가 고용계획 표 데이터

결과는 content["_tables"] 에 저장되며 render_agent._build_injector_content()에서 소비.
"""
