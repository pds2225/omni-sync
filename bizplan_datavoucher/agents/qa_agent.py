...
데이터바우처 전용 검사 항목:
  ...
  E05: 수행인력 참여율과 사업비 인건비 미기재 (팀원 수 검사)
  E06: 핵심 서술 섹션(problem/solution/market_status) 중 비어 있는 항목 존재

  [경고 — 수정 권고]
  W01: 과제 목표(goal_100char) 100자 초과
  W02: 단일 사업비 항목 70% 초과 (편중 위험)
  W03: 추진일정 비중 합계 ≠ 100%
  W04: 수행인력 참여율이 10% 미만이거나 100% 초과인 멤버 존재
  W05: 이미지 파일이 없어 삽입 건너뜀
  W06: 과제명이 50자를 초과함 (가독성 저하)
  W07: 기대효과(expected_effect) 항목이 비어 있음
...
class QAAgent:
    ...
    def run(self, content: dict, output_path: str = "") -> dict:
        ...
        if output_path and not os.path.exists(output_path):
            if not content.get("_tables", {}).get("dry_run"):
